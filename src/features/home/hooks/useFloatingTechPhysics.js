...
export function useFloatingTechPhysics(showMain, moonRefs, profileMassRef) {
  ...
  useEffect(() => {
    ...
    const initializePhysics = () => {
      ...
      if (moons.length === 0) {
        // Floating moon nodes can mount after first render.
      ...
      const tick = () => {
        ...
        const profileObstacle = profileRect
          ? {
              cx: profileRect.left + profileRect.width / 2,
              cy: profileRect.top + profileRect.height / 2,
  ...
  }, [showMain, moonRefs, profileMassRef]);
}
